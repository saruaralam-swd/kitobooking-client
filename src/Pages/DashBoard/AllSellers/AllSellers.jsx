...
const AllSellers = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://kitobooking-server.vercel.app/allSellers?email=${user?.email}`,
        {
          headers: {
  ...
  const handleSellerVerify = (sellerEmail) => {
    fetch(
      `https://kitobooking-server.vercel.app/verifySeller/${sellerEmail}?email=${user?.email}`,
      {
        method: "PUT",
        headers: {
          authorization: `Bearer ${localStorage.getItem("accessToken")}`,
  ...
  const handleSellerDelete = (id) => {
    ...
    if (permission) {
      fetch(
        `https://kitobooking-server.vercel.app/seller/${id}?email=${user?.email}`,
        {
          method: "DELETE",
          headers: {
            authorization: `Bearer ${localStorage.getItem("accessToken")}`,
